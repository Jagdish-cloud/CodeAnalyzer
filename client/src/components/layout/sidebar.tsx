...
export function Sidebar({ isOpen, onClose }: SidebarProps) {
  ...
  const [expandedGroups, setExpandedGroups] = useState<string[]>(['institution']);
      ...
      return (
        <Link key={item.id} href={item.url}>
          <Button
            ...
            className={cn(
              "w-full justify-start p-2 h-auto text-sm font-normal",
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div className={cn(
        "fixed left-0 top-0 h-full w-64 bg-background border-r border-border z-50 transform transition-transform duration-200 ease-in-out lg:translate-x-0",
